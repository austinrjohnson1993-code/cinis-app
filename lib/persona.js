...
export function buildPersonaPrompt(profile) {
  ...
  const staples = profile.nutrition_staples || []
  const nutritionLine = staples.length > 0
    ? `- Nutrition: User regularly consumes: ${staples.join(', ')}. Factor this into energy, focus, and productivity coaching where relevant.`
    : ''
