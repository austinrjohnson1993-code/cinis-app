...
export default function Dashboard() {
  ...
  const [incomeForm, setIncomeForm] = useState({ type: 'Salary', amount: '', frequency: 'monthly' })
  ...
  const fetchHabitsList = async (userId) => {
  ...
  const toggleHabitCompletion = async (habitId) => {
    ...
    if (res.ok) {
      ...
      setHabitsList(prev =>
        prev.map(h => h.id === habitId ? { ...h, completedToday } : h)
      )
    }
  }
      ...
      <div className={styles.appShell}>
        ...
        <main className={styles.main}>
              ...
              <div className={styles.settingsSection} style={{ paddingTop: '24px' }}>
                ...
                <div className={styles.settingsCard}>
                  {[
                    ...
                    { key: 'midday', label: 'Midday check-in', val: notifMidday, set: setNotifMidday, time: middayTime, setTime: setMiddayTime },
                    { key: 'evening', label: 'Evening wrap-up', val: notifEvening, set: setNotifEvening, time: eveningTime, setTime: setEveningTime },
                  ].map(({ key, label, val, set, time, setTime }) => (
                    <div key={key} className={`${styles.settingsRow}`} style={{ flexWrap: 'wrap', gap: '12px' }}>
                      <div className={styles.settingsRowLeft} style={{ flex: 1 }}>
                        <span className={styles.settingsRowLabel}>{label}</span>
                        <span className={styles.settingsRowSub}>Scheduled at {time}</span>
                      </div>
                      <div style={{ display: 'flex', gap: '8px', alignItems: 'center' }}>
                        <input type="time" value={time} onChange={e => setTime(e.target.value)} style={{ padding: '6px 8px', borderRadius: '4px', border: '1px solid rgba(255,102,68,0.2)', background: 'rgba(42,24,16,0.4)', color: '#F0EAD6', fontFamily: 'Figtree, sans-serif', fontSize: '13px' }} />
                        <button onClick={() => set(v => !v)}
                          className={`${styles.notifToggle} ${val ? styles.notifToggleOn : ''}`} style={{ flexShrink: 0 }} />
                      </div>
                    </div>
                  ))}
          ...
          <TabErrorBoundary tabName="Habits">
            <div className={styles.view}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: '24px', paddingBottom: '12px', borderBottom: '1px solid rgba(240,234,214,0.1)' }}>
                <h1 className={styles.greetingText}>Habits</h1>
                <button onClick={() => setShowHabitModal(true)} className={styles.addTaskBtn}>
                  + Add habit
                </button>
              </div>

              {/* Loading skeleton */}
              {habitsLoading && habitsList.length === 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {[1, 2, 3].map(idx => (
                    <div key={idx} style={{ display: 'flex', alignItems: 'center', padding: '16px', background: 'rgba(255,102,68,0.05)', borderRadius: '12px', border: '1px solid rgba(255,102,68,0.15)', gap: '12px', opacity: 0.5 }}>
                      <div style={{ flex: 1 }}>
              ...
              {/* Empty state */}
              {habitsList.length === 0 && !habitsLoading && (
                <div className={styles.emptyState}>
                  ...
                  <button onClick={() => setShowHabitModal(true)} className={styles.emptyAddBtn}>
              ...
              {habitsList.length > 0 && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: '12px' }}>
                  {habitsList.map(habit => (
                    <div key={habit.id} style={{ display: 'flex', alignItems: 'center', padding: '16px', background: 'rgba(255,102,68,0.05)', borderRadius: '12px', border: '1px solid rgba(255,102,68,0.15)' }}>
                      <div style={{ flex: 1 }}>
                        <div style={{ fontSize: '16px', fontWeight: 600, color: '#F0EAD6', marginBottom: '4px' }}>{habit.name}</div>
                        <div style={{ display: 'flex', alignItems: 'center', gap: '8px', flexWrap: 'wrap' }}>
                          <span style={{ fontSize: '12px', color: 'rgba(240,234,214,0.5)', textTransform: 'capitalize' }}>{habit.frequency}</span>
                          {habit.habit_type === 'break' && (
                            <span style={{ fontSize: '11px', color: 'rgba(240,234,214,0.4)', fontWeight: 500 }}>Breaking</span>
                          )}
                          {habit.streak > 0 && (
                      ...
                      <button
                        ...
                        style={{ width: '40px', height: '40px', borderRadius: '50%', border: habit.completedToday ? 'none' : '2px solid rgba(240,234,214,0.3)', background: habit.completedToday ? '#E8321A' : 'transparent', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', transition: 'all 200ms ease' }}
                      >
                        {habit.completedToday && <span style={{ fontSize: '18px' }}>{habit.habit_type === 'break' ? '💪' : '✓'}</span>}
                      </button>
                    </div>
                  ))}
                </div>
              ...
              {/* Add habit modal */}
              {showHabitModal && (
                <div className={styles.modalOverlay} onClick={(e) => { if (e.target === e.currentTarget) setShowHabitModal(false) }}>
                  <div className={styles.modal}>
                    <div className={styles.modalHeader}>
                      <h2 className={styles.modalTitle}>Add habit</h2>
                      <button onClick={() => setShowHabitModal(false)} style={{ background: 'none', border: 'none', color: '#F0EAD6', fontSize: '24px', cursor: 'pointer' }}>×</button>
                    </div>
                    <form onSubmit={handleAddHabit} style={{ padding: '20px' }}>
                      <div className={styles.fieldGroup}>
                        <label className={styles.fieldLabel}>Type</label>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {[{ val: 'build', label: 'Build a habit' }, { val: 'break', label: 'Break a habit' }].map(({ val, label }) => (
                            <button key={val} type="button" onClick={() => setNewHabitType(val)} style={{ flex: 1, padding: '10px', borderRadius: '6px', border: newHabitType === val ? '2px solid #FF6644' : '1px solid rgba(240,234,214,0.2)', background: newHabitType === val ? 'rgba(255,102,68,0.1)' : 'transparent', color: newHabitType === val ? '#F0EAD6' : 'rgba(240,234,214,0.5)', cursor: 'pointer', fontSize: '13px', fontWeight: newHabitType === val ? 600 : 400, transition: 'all 200ms ease' }}>{label}</button>
                          ))}
                        </div>
                      </div>
                      <div className={styles.fieldGroup}>
                        <label className={styles.fieldLabel}>Habit name</label>
                        <input type="text" placeholder="e.g. Meditate" value={newHabitName} onChange={(e) => setNewHabitName(e.target.value)} required className={styles.fieldInput} autoFocus />
                      </div>
                      <div className={styles.fieldGroup}>
                        <label className={styles.fieldLabel}>Frequency</label>
                        <div style={{ display: 'flex', gap: '8px' }}>
                          {['daily', 'weekly'].map(freq => (
                            <button key={freq} type="button" onClick={() => setNewHabitFreq(freq)} style={{ flex: 1, padding: '10px', borderRadius: '6px', border: newHabitFreq === freq ? '2px solid #FF6644' : '1px solid rgba(240,234,214,0.2)', background: newHabitFreq === freq ? 'rgba(255,102,68,0.1)' : 'transparent', color: '#F0EAD6', cursor: 'pointer', textTransform: 'capitalize', fontWeight: newHabitFreq === freq ? 600 : 400 }}>{freq}</button>
                          ))}
                        </div>
                      </div>
                      <div className={styles.fieldGroup}>
                        ...
                        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: '8px' }}>
                          {habitColorOptions.map(color => (
                            <button key={color} type="button" onClick={() => setNewHabitColor(color)} style={{ width: '100%', aspectRatio: '1', borderRadius: '6px', background: color, border: newHabitColor === color ? '3px solid #F0EAD6' : 'none', cursor: 'pointer', transition: 'all 200ms ease' }} />
                          ))}
                        </div>
                      </div>
                      <button type="submit" className={styles.addTaskBtn} style={{ width: '100%', marginTop: '16px' }}>Save habit</button>
                    </form>
                  </div>
                </div>
              )}
          ...
          <TabErrorBoundary tabName="Finance">
            <div className={styles.view}>
              ...
              <div className={styles.financeSubTabs}>
                {[['bills',<Receipt size={18} />,'Bills'],['budget',<Scales size={18} />,'Budget'],['knowledge',<Books size={18} />,'Knowledge'],['insights',<Robot size={18} />,'Insights']].map(([id, icon, label]) => (
            ...
            <div className={styles.modal}>
              ...
              <form onSubmit={addBill} className={styles.modalForm}>
                ...
                {newBillFrequency === 'bimonthly' && (
                  <>
                    <div className={styles.fieldGroup}>
                      <label className={styles.fieldLabel}>First date <span className={styles.fieldLabelOptional}>(optional)</span></label>
                      <input type="number" placeholder="e.g. 1" min="1" max="31"
                        value={newBillDate1} onChange={e => setNewBillDate1(e.target.value)}
                        className={styles.fieldInput} />
                    </div>
                    <div className={styles.fieldGroup}>
                      <label className={styles.fieldLabel}>Second date <span className={styles.fieldLabelOptional}>(optional)</span></label>
                      <input type="number" placeholder="e.g. 15" min="1" max="31"
                        value={newBillDate2} onChange={e => setNewBillDate2(e.target.value)}
                        className={styles.fieldInput} />
                    </div>
                  </>
                )}
                ...
                <div className={styles.fieldGroup}>
                  <label className={styles.fieldLabel}>Notes <span className={styles.fieldLabelOptional}>(optional)</span></label>
                  <textarea placeholder="Add any notes about this bill..."
                    value={newBillNotes} onChange={e => setNewBillNotes(e.target.value)}
                    className={styles.fieldInput} style={{ minHeight: '80px', resize: 'vertical' }} />
                </div>
                <div className={styles.fieldGroup}>
